  ...
  return this.each(function() {
    ...
    */
    function parse_source() {
      ...
      // reset optimization var
      leftmost_available_space = 0;
    ...
    */
    function available_space($write_float) {
      ...
      // skip ahead to the larger of either our current pointer or the
      // optimization variable leftmost_available_space. and skip that ahead, if necessary.
      space_coords.left = Math.max(space_coords.left, leftmost_available_space);
      leftmost_available_space = space_coords.left;
      ...
      while (true) {
        ...
        $sandbox.find(HZ_FLOAT).each(function() {
          ...
          if ((space_coords.left < float_right) && ((space_coords.left + required_width) > float_left)) {
            ...
            // if float is a full-height, increment optimization var and skip this column
            if ($float.hasClass('hz_full_height')) {
              column_occupied = true;
              
              // optimization! ONLY increment for contiguous full-height floats
              if (leftmost_available_space == space_coords.left) {
                leftmost_available_space += COLUMN_WIDTH;
              }
